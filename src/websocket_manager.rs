...
async fn pull_subscribe_message(
    next: Option<Result<Message, Error>>,
    ...
    if let Some(Ok(Message::Text(text))) = next {
        if let Ok(subscribe_message) = serde_json::from_str::<SocketMessage>(text.as_str()) {
            if let Some(listener) = listeners
                .get_listener(String::from(
                    subscribe_message.data.id.as_str().unwrap_or(""),
                ))
                .await
            {
