...
async fn handle_request(mut request: Request<Body>, listeners: Arc<ListenerManager>) -> Result<Response<Body>, Error> {
    ...
    if hyper_tungstenite::is_upgrade_request(&request) {
        ...
        if request.uri().to_string().starts_with("/simstatus.json") {
            ...
            if let Ok(post) = serde_json::from_str::<CustomGamePost>(String::from_utf8(hyper::body::to_bytes(request.into_body()).await?.to_vec())?.as_str()) {
                ...
                if split.len() == 4 {
                    ...
                    if split.len() == 2 {
                        ...
                        if let Some(address) = crate::utils::to_wss_address(&split[1].parse().unwrap()) {
                            dbg!(address.clone());
                            ...
                            return Ok(match res {
                                None => {
                                    Response::builder()
                                        ...
                                        .body(Body::from("{\"status\":\"error\",\"error\":\"error\"}")).expect("failure building body")
                                }
                                Some(result) => {
                                    match result {
                                        ListenerAdditionResponse::CannotJoin => {
                                            Response::builder()
                                                .header("Access-Control-Allow-Origin", "*")
                                                .status(StatusCode::OK)
                                                .body(Body::from("{\"error\":\"can't connect\"}")).expect("failure building body")
                                        }
                                        ListenerAdditionResponse::Success => {
                                            Response::builder()
                                                .header("Access-Control-Allow-Origin", "*")
                                                .status(StatusCode::OK)
                                                .body(Body::from("{\"status\":\"success\"}")).expect("failure building body")
                                        }
                                        ListenerAdditionResponse::AlreadyExists => {
                                            Response::builder()
                                                .header("Access-Control-Allow-Origin", "*")
                                                .status(StatusCode::OK)
                                                .body(Body::from("{\"status\":\"success\"}")).expect("failure building body")
                                        }
                                        ListenerAdditionResponse::BadFormat => {
                                            Response::builder()
                                                .header("Access-Control-Allow-Origin", "*")
                                                .status(StatusCode::BAD_REQUEST)
                                                .body(Body::from("{\"status\":\"error\",\"error\":\"invalid format\"}")).expect("failure building body")
                                        }
                                        ListenerAdditionResponse::IsPublic => {
                                            Response::builder()
                                                .header("Access-Control-Allow-Origin", "*")
                                                .status(StatusCode::OK)
                                                .body(Body::from("{\"status\":\"success\"}")).expect("failure building body")
